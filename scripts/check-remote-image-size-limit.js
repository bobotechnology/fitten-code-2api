...
const oversizedPng = Buffer.alloc(6 * 1024 * 1024, 65);
...
const fixtureServer = http.createServer((request, response) => {
  ...
  response.setHeader('Content-Length', String(oversizedPng.length));
  response.end(oversizedPng);
});

fixtureServer.listen(3030, '127.0.0.1', () => {
    ...
    try {
        ...
        messages: [
          {
            role: 'user',
            content: [
              { type: 'input_image', image_url: 'http://127.0.0.1:3030/large.png' },
      ...
      finish(0, {
        ...
        rejected_as_expected:
          response.status === 400 &&
          parsed?.error?.code === 'image_too_large' &&
